//
// Created by wendell on 2024/9/22.
//
#include "avl.h"
...
// fix imbalanced nodes and maintain invariants util the root is reached
...
AVLNode* avl_del(AVLNode* node) {
    if (node->right == NULL) {
        // no right subtree, replace the node with the left tree
...
AVLNode* avl_offset(AVLNode* node, int64_t offset) {
    int pos = 0;    // relative to the starting node
    while(offset != pos) {
